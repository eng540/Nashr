...
async def run_discovery_job(job_id: UUID) -> None:
    """Execute one persisted discovery job with a fresh DB session and topic checkpoints."""
    if not await _claim_job(job_id):
        return

    discoverer = DiscoverBook(GeminiBookMapper(), GeminiTopicMaterialDiscoverer())
    ...
    try:
        ...
        async with SessionFactory() as session:
            document = await discoverer.prepare_document(session, source_id)

        await _update_job(job_id, stage=DiscoveryJobStage.BUILDING_BOOK_MAP)
        async with SessionFactory() as session:
            book_map = await discoverer.build_book_map(session, source_id, document)
        ...
        for topic in book_map.topics:
            ...
            async with SessionFactory() as session:
                await discoverer.discover_topic(session, source_id, topic.id, document)
            await _progress(job_id, source_id, None)
