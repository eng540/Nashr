...
from app.domain.extraction import ExtractedIdea, IBookMapper, IExtractor, ITopicMaterialDiscoverer
...
class FakeExtractor(IExtractor):
    ...
    async def extract(self, source: Source) -> list[ExtractedIdea]:
        return [ExtractedIdea(position=i, title=f"Mock material {i}", content=f"Mock content {i} from {source.filename}.", original_text=f"Original mock text {i}", source_reference=f"page {i}", kind="mock") for i in range(1, self.count + 1)]
...
class FakeTopicMaterialDiscoverer(ITopicMaterialDiscoverer):
    ...
    async def discover_topic(self, source: Source, topic: BookTopic) -> list[ExtractedIdea]:
        return [ExtractedIdea(position=i, title=f"Topic material {i}", content=f"Topic content {i}", original_text=f"Original topic text {i}", source_reference=f"page {i}", kind=("حكمة" if i == 1 else "نوع مكتشف")) for i in range(1, self.count + 1)]
